...
export default function AppsPage() {
  ...
  const categories = [
    { name: t("apps.categories.all"), count: 250 },
    { name: t("apps.categories.development"), count: 85 },
    { name: t("apps.categories.chat"), count: 62 },
    { name: t("apps.categories.writing"), count: 45 },
    { name: t("apps.categories.research"), count: 38 },
    { name: t("apps.categories.localAi"), count: 20 },
  ];
  return (
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <main className="flex-1">
        <div className="mx-auto max-w-7xl px-6 md:px-8 py-12">
          {/* Header */}
          <div className="mb-10">
            <h1 className="text-4xl font-bold mb-3">{t("apps.title")}</h1>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-12">
            <div className="rounded-xl border border-border bg-card p-5">
              ...
              <p className="text-sm text-muted-foreground">{t("apps.stats.totalApps")}</p>
            </div>
            <div className="rounded-xl border border-border bg-card p-5">
              <p className="text-3xl font-bold">4.2M+</p>
              <p className="text-sm text-muted-foreground">{t("apps.stats.users")}</p>
            </div>
            <div className="rounded-xl border border-border bg-card p-5">
              <p className="text-3xl font-bold">30T+</p>
              <p className="text-sm text-muted-foreground">{t("apps.stats.tokensPerMonth")}</p>
            </div>
            <div className="rounded-xl border border-border bg-card p-5">
              <p className="text-3xl font-bold">300+</p>
              <p className="text-sm text-muted-foreground">{t("apps.stats.modelsUsed")}</p>
            </div>
          </div>

          {/* Featured Apps */}
          <div className="mb-8">
            <h2 className="text-2xl font-bold mb-6">{t("apps.featured")}</h2>
          ...
          <div className="rounded-2xl border border-border bg-card p-8 text-center">
            <h2 className="text-2xl font-bold mb-3">{t("apps.buildWith.title")}</h2>
            <p className="text-muted-foreground mb-6 max-w-lg mx-auto">
              {t("apps.buildWith.description")}
            </p>
            <Link
              href="/docs"
